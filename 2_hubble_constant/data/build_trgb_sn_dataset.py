"""
build_cats_sn_dataset.py
Build CATS-consistent SN dataset: TRGB_y.npy, TRGB_C.npy, TRGB_labels.csv

Calibrators
-----------
All SH0ES R22 calibrators (from SH0ES_y_labels.csv) PLUS the three CATS
calibrator galaxies not in SH0ES: NGC 1316, NGC 1404, NGC 4526.
...
criteria: redshift 0.023 < z_HD < 0.15, x1 and c within Pantheon+ natural
range (CATS used -3 < x1 < 3, -0.3 < c < 0.3 — these are non-restrictive
given the actual Pantheon+ ranges), and IS_CALIBRATOR == 0.  Any SN already
assigned as a calibrator is excluded from the HF sample.
...
CATS_extended_y_labels.csv  — built by make_cats_labels.py (already present)

Outputs (saved to this directory)
-----------------------------------
TRGB_y.npy      — y vector (calibrator m_b_corr + HF Hubble intercepts)
TRGB_C.npy      — covariance submatrix for selected rows
TRGB_labels.csv — row metadata (type, host, label)

To use in demo_mu_to_H0.ipynb, load these files instead of the SH0ES22_partial_*
files in cell 2.
"""
...
CATS_LABELS = os.path.join(DATA_DIR, 'TRGB_extended_y_labels.csv')
...
print("Loading CATS extended labels...")
cats_labels = pd.read_csv(CATS_LABELS)

# ── Build calibrator set ──────────────────────────────────────────────────────
# CATS_extended_y_labels mixes two index systems in 'Unnamed: 0':
#   - SH0ES rows: FITS-file indices (3130+), not valid Pantheon+ row indices
#   - New rows (n1316, n1404, n4526): Pantheon+ row indices (4–55)
# Solution: match ALL calibrators to Pantheon+ by CID + IDSURVEY key, which
# is unambiguous and present in both files.
cal_labels = cats_labels[cats_labels['calib_host'].notna()].copy()

# Build a CID_IDSURVEY → Pantheon+ row index lookup
...
print(f"\nExtracting {len(all_idx)}×{len(all_idx)} covariance submatrix...")
...
print(f"\nTo use in demo_mu_to_H0.ipynb, replace the load paths in cell 2:")
print(f"  y      = np.load('data/TRGB_y.npy')")
print(f"  C      = np.load('data/TRGB_C.npy')")
print(f"  labels = pd.read_csv('data/TRGB_labels.csv')")
